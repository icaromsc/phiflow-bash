...
#print("ok carregou pacotes")

# test if there is at least one argument: if not, return an error
if (length(args)==0) {
  stop("At least one argument must be supplied (filepath dir)", call.=FALSE)
} else if (length(args)==1) {
  # default output dir
  args[2] = "~/Documentos/metatranscriptomics_benchmarking/"
}
...
file_list <- list.files(path=paste0(getwd(),"/K2_results"), pattern = "\\.k2.report$")
print(file_list)
...
for (i in 1:length(file_list)) {
  ...
  temp_kraken2_table <- read.table(paste0("K2_results/",file_list[i]), sep = "\t", header = F,fill = TRUE)
